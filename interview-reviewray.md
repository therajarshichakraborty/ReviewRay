...
> *"Here is how I explain ReviewRay naturally when asked 'Tell me about your project' during an interview."*
...
**Answer**: Git diffs only show modified lines in isolation. A bug often occurs because a modified function breaks callers or dependencies in *unmodified* files. Passing only the diff leads to shallow reviews. Fulfilling the prompt with full repo context via vector search (Pinecone) supplies cross-file context (e.g. function signatures, type definitions, helper utilities) while keeping prompt tokens focused and cost-effective.
...
- **Interviewer Cross Question**: *"What happens if 1,000 PRs are opened simultaneously? Won't your Inngest workers hit LLM rate limits or Pinecone quota limits?"*
- **Practical Answer**: 
...
- **Interviewer Cross Question**: *"How do you split code files into chunks? What happens if a function definition is split right down the middle?"*
- **Practical Answer**: 
...
- **Interviewer Cross Question**: *"Developers are sensitive about sending proprietary code to third-party AI APIs. How does ReviewRay handle code privacy?"*
- **Practical Answer**:
